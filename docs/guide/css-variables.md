...
CSS 变量具有很高的动态性，业界并没有很可靠的 polyfill 方案，因此 vigor-design-mobile 从 `5.14.0` 版本开始，官方提供了一份补丁 CSS：

```js
import 'vigor-design-mobile/bundle/css-vars-patch.css'
```

你可以在项目中引入它，来确保在 iOS 9 下组件样式也不会乱掉。

> 对于 `2x` 项目，请引入对应的 `vigor-design-mobile/2x/bundle/css-vars-patch.css`
